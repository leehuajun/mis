...
import com.sunjet.mis.module.report.service.CarForecastDemandPlanningService;
...
/**
 * @author SUNJET-YFS
 * @Title: CarForecastDemandPlanningVM
 * @ProjectName mis
 * @Description: 客厢车月预测客厢车销售需求计划 - 客厢车
 * @date 2019/4/2810:34
 */
public class CarForecastDemandPlanningVM extends ListVM<CarForecastDemandPlanningView> {

    @WireVariable
    private CarForecastDemandPlanningService carForecastDemandPlanningService;

    @Getter
    @Setter
    private CarForecastDemandPlanningView carForecastDemandPlanningView = new CarForecastDemandPlanningView();
    @Getter
    @Setter
    private List<CarForecastDemandPlanningView> carForecastDemandPlanningViews = new ArrayList<>();
    @Getter
    @Setter
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
           // TableHeaderInfo.builder().label("行号").width("40px").build(),
            TableHeaderInfo.builder().label("生产计划时间").width("100px").build(),
            TableHeaderInfo.builder().label("产地").width("60px").build(),
            TableHeaderInfo.builder().label("产品类别").width("100px").build(),
            TableHeaderInfo.builder().label("产品名称").width("120px").build(),
            TableHeaderInfo.builder().label("车型").width("100px").build(),
            TableHeaderInfo.builder().label("非ABS-VSN").width("100px").build(),
            TableHeaderInfo.builder().label("ABS-VSN").width("70px").build(),
            TableHeaderInfo.builder().label("上月底成品预计结余").width("70px").build(),
            TableHeaderInfo.builder().label("月生产计划").width("70px").build(),
            TableHeaderInfo.builder().label("采购量").width("70px").build()
    );
    @Init
    public void init() {
        this.setSearchFormHeight(49);
        this.initPermissionStatus(CarForecastDemandPlanningView.class.getSimpleName());
...
//        }
        refreshFirstPage(carForecastDemandPlanningView, Order.DESC, "production");
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<CarForecastDemandPlanningView> dataSet = null;
        ...
        for (CarForecastDemandPlanningView detailItem : dataSet) {
    ...
    public void getPageList() {
        ...
        this.setPageResult(carForecastDemandPlanningService.getPageList(this.getPageParam()));
    }
...
//        }
    ...
    @NotifyChange("*")
    public void reset() {
        this.carForecastDemandPlanningViews.clear();
        //this.carForecastDemandPlanningView.setZtime(null);
        this.carForecastDemandPlanningView.setAbsvsn(null);
        this.carForecastDemandPlanningView.setStartDate(null);
        this.carForecastDemandPlanningView.setEndDate(null);
        this.getPageList();
    }
    ...
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(carForecastDemandPlanningView);
        getPageList();
    }
    ...
    public String genStyle(CarForecastDemandPlanningView entity) {
