...
import com.sunjet.mis.module.warehouse.service.ChassisOutboundQingDaoService;
 ...
 * 青岛底盘出库
 */
public class ChassisOutboundQingDaoVM extends ListVM<ChassisOutboundQINGDAOEntity> {

    @WireVariable
    private ChassisOutboundQingDaoService chassisOutboundQingDaoService;

    @Getter
    @Setter
    private ChassisOutboundQINGDAOEntity chassisOutboundQINGDAOEntity = ChassisOutboundQINGDAOEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ChassisOutboundQINGDAOEntity selectedChassis;
    ...
    @Getter
    private List<ChassisOutboundQINGDAOEntity> importEntities = new ArrayList<>();

    @Getter
    private List<ChassisOutboundQINGDAOEntity> importList = new ArrayList<>();
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("底盘道位").build(),
            TableHeaderInfo.builder().label("底盘VSN").build(),
            TableHeaderInfo.builder().label("青岛车型标识").build(),
            TableHeaderInfo.builder().label("生产批号").build(),
            TableHeaderInfo.builder().label("司机码").build(),
            TableHeaderInfo.builder().label("产品").build(),
            TableHeaderInfo.builder().label("过站时间").build(),
            TableHeaderInfo.builder().label("修改次数").build(),
            TableHeaderInfo.builder().label("最后操作").build(),
            TableHeaderInfo.builder().label("扫描员").build()
    ...
    public void init() {
        ...
        this.initPermissionStatus(ChassisOutboundQINGDAOEntity.class.getSimpleName());
        refreshFirstPage(chassisOutboundQINGDAOEntity);
        getPageList();
    }
    ...
    public void simpleSearch(){
        chassisOutboundQINGDAOEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(chassisOutboundQingDaoService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(chassisOutboundQINGDAOEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedChassis = chassisOutboundQINGDAOEntity.builder().build();
            } else {
                this.selectedChassis = chassisOutboundQingDaoService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ChassisOutboundQingDaoVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ChassisOutboundQingDaoVM.this, "selectedChassis");
...
//        this.parentOrgs.clear();
    ...
    @NotifyChange("*")
    public void submit() {
        chassisOutboundQingDaoService.save(this.selectedChassis);
        this.selectedChassis = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = chassisOutboundQingDaoService.deleteById(id);
                if(bl==true){
...
//        this.importEntities = chassisOutboundQingDaoService.saveAll(this.importEntities);
//        int errs = this.importEntities.size();
        int count = this.importList.size();
        this.importList = chassisOutboundQingDaoService.saveAll(this.importList);
        int errs = this.importList.size();
        if(errs>0){
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ChassisOutboundQINGDAOEntity.class);
            for(ChassisOutboundQINGDAOEntity outqd:importEntities){
                if(StringUtils.isNotBlank(outqd.getVin()) && StringUtils.isNotBlank(outqd.getStandingTime().toString())){
                    importList.add(outqd);
                }
            }
            // 删除上传来的临时文件
...
////
            ...
            Class<ChassisOutboundQINGDAOEntity> clazz = ChassisOutboundQINGDAOEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ChassisOutboundQINGDAOEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.chassisOutboundQINGDAOEntity.setVin("");
        this.chassisOutboundQINGDAOEntity.setVsn("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/青岛底盘出库.xlsx";
            ...
            Filedownload.save(buffer, null, "青岛底盘出库.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ChassisOutboundQINGDAOEntity> dataSet = null;
        ...
        for (ChassisOutboundQINGDAOEntity detailItem : dataSet) {
            Map<String, Object> map = BeanUtils.transBean2Map(detailItem);
            maps.add(map);
        }
        List<String> keyList = new ArrayList<>();
        keyList.add("vin");
        keyList.add("chassisBit");
        keyList.add("vsn");
        keyList.add("modelLogo");
        keyList.add("sharesProduceNumber");
        keyList.add("driverCode");
        keyList.add("product");
        keyList.add("standingTime");
        keyList.add("modifyNumber");
        keyList.add("lastOperation");
        keyList.add("scanMember");
