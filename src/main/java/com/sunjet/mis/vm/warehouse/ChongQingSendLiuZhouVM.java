...
import com.sunjet.mis.module.warehouse.service.ChongQingSendLiuZhouService;
 ...
 * 青岛底盘出库
 */
public class ChongQingSendLiuZhouVM extends ListVM<ChongQingSendLiuZhouEntity> {

    @WireVariable
    private ChongQingSendLiuZhouService chongQingSendLiuZhouService;

    @Getter
    @Setter
    private ChongQingSendLiuZhouEntity chongQingSendLiuZhouEntity = ChongQingSendLiuZhouEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ChongQingSendLiuZhouEntity selectedSend;
    ...
    @Getter
    private List<ChongQingSendLiuZhouEntity> importEntities = new ArrayList<>();


    @Init
    public void init() {
        this.setMode(OpenMode.DIALOG);
        this.initPermissionStatus(ChongQingSendLiuZhouEntity.class.getSimpleName());
        refreshFirstPage(chongQingSendLiuZhouEntity);
        getPageList();
    }
    ...
    public void simpleSearch(){
        chongQingSendLiuZhouEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(chongQingSendLiuZhouService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(chongQingSendLiuZhouEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedSend = chongQingSendLiuZhouEntity.builder().build();
            } else {
                this.selectedSend = chongQingSendLiuZhouService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ChongQingSendLiuZhouVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ChongQingSendLiuZhouVM.this, "selectedSend");
...
//        this.parentOrgs.clear();
    ...
    @NotifyChange("*")
    public void submit() {
        chongQingSendLiuZhouService.save(this.selectedSend);
        this.selectedSend = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = chongQingSendLiuZhouService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        ...
        this.importEntities = chongQingSendLiuZhouService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ChongQingSendLiuZhouEntity.class);
...
////
            ...
            Class<ChongQingSendLiuZhouEntity> clazz = ChongQingSendLiuZhouEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ChongQingSendLiuZhouEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.chongQingSendLiuZhouEntity.setVin("");
        this.chongQingSendLiuZhouEntity.setVsn("");
        this.chongQingSendLiuZhouEntity.setVehicleModel("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/重庆发柳州.xlsx";
            ...
            Filedownload.save(buffer, null, "重庆发柳州.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ChongQingSendLiuZhouEntity> dataSet = null;
        ...
        for (ChongQingSendLiuZhouEntity detailItem : dataSet) {
