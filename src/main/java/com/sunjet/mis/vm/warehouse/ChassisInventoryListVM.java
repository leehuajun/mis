...
import com.sunjet.mis.module.warehouse.service.ChassisInventoryService;
...
public class ChassisInventoryListVM extends ListVM<ChassisInventoryEntity> {

    @WireVariable
    private ChassisInventoryService chassisInventoryService;

    @Getter
    @Setter
    private ChassisInventoryEntity chassisInventoryEntity = ChassisInventoryEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ChassisInventoryEntity selectedChassisInventory;
    ...
    @Getter
    private List<ChassisInventoryEntity> importEntities = new ArrayList<>();


    @Init
    public void init() {
        this.setMode(OpenMode.DIALOG);
        this.initPermissionStatus(ChassisInventoryEntity.class.getSimpleName());
        refreshFirstPage(chassisInventoryEntity);
        getPageList();
    }
    ...
    public void simpleSearch(){
        //chassisInventoryEntity.setVin(this.getKeyword());
        getPageList();
    }
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("发运日期").build(),
            TableHeaderInfo.builder().label("生产批次").build(),
            TableHeaderInfo.builder().label("颜色").build(),
            TableHeaderInfo.builder().label("合格证号").build(),
            TableHeaderInfo.builder().label("基地代码").build(),
            TableHeaderInfo.builder().label("制造日期").build(),
            TableHeaderInfo.builder().label("车型代码").build(),
            TableHeaderInfo.builder().label("数据来源").build()
...
//    @Command
    public void getPageList() {
        this.setPageResult(chassisInventoryService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(chassisInventoryEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedChassisInventory = chassisInventoryEntity.builder().build();
            } else {
                this.selectedChassisInventory = chassisInventoryService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ChassisInventoryListVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ChassisInventoryListVM.this, "selectedChassisInventory");
...
//        this.parentOrgs.clear();
        this.selectedChassisInventory = null;
        this.showForm = false;
    ...
    @NotifyChange("*")
    public void submit() {
        chassisInventoryService.save(this.selectedChassisInventory);
        this.selectedChassisInventory = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = chassisInventoryService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        String dataSource = this.uploadFilename.substring(0,this.uploadFilename.indexOf("."));
        for(ChassisInventoryEntity ci:this.importEntities){
            ci.setDataSource(dataSource);
        }

        int count = this.importEntities.size();
        this.importEntities = chassisInventoryService.saveAll(this.importEntities);
        int errs = this.importEntities.size();
        if(errs>0){
            ZkUtils.showInformation(String.format("导入失败！总记录数：%d, 没有问题数据：%d条,有问题数据：%d条。错误数据展示如下 ", count, count - errs,errs), "系统提示");
        }else{
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ChassisInventoryEntity.class);
            List<ChassisInventoryEntity> deleteList = new ArrayList<>();
            for(ChassisInventoryEntity entity:importEntities){
                if(entity.getVin()==null){
                    deleteList.add(entity);
                }
            }
            if(deleteList.size()>0){
                importEntities.removeAll(deleteList);
            }
...
////
            ...
            Class<ChassisInventoryEntity> clazz = ChassisInventoryEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ChassisInventoryEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.chassisInventoryEntity.setVin("");
        this.chassisInventoryEntity.setVsn("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/底盘库存.rar";
            ...
            Filedownload.save(buffer, null, "底盘库存.rar");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ChassisInventoryEntity> dataSet = null;
        ...
        for (ChassisInventoryEntity detailItem : dataSet) {
        ...
        keyList.add("shipmentData");
        keyList.add("batchNumber");
        keyList.add("color");
        keyList.add("qualifiedCertificate");
        keyList.add("baseCode");
        keyList.add("makeData");
