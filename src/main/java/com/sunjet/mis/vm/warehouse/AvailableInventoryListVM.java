...
import com.sunjet.mis.module.warehouse.service.AvailableInventoryService;
...
public class AvailableInventoryListVM extends ListVM<AvailableInventoryEntity> {

    @WireVariable
    private AvailableInventoryService availableInventoryService;

    @Getter
    @Setter
    private AvailableInventoryEntity availableInventoryEntity = AvailableInventoryEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private AvailableInventoryEntity selectedAvailableInventory;
    ...
    @Getter
    private List<AvailableInventoryEntity> importEntities = new ArrayList<>();
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("VSN").width("40px").build(),
            TableHeaderInfo.builder().label("VIN").width("40px").build(),
            TableHeaderInfo.builder().label("库存状态").width("40px").build(),
            TableHeaderInfo.builder().label("仓库区域").width("40px").build(),
            TableHeaderInfo.builder().label("产品型号").width("40px").build(),
            TableHeaderInfo.builder().label("产品名称").width("40px").build(),
            TableHeaderInfo.builder().label("颜色").width("40px").build(),
            TableHeaderInfo.builder().label("订单号").width("40px").build(),
            TableHeaderInfo.builder().label("库位").width("40px").build(),
            TableHeaderInfo.builder().label("车辆编号").width("40px").build(),
            TableHeaderInfo.builder().label("入库日期").width("40px").build(),
            TableHeaderInfo.builder().label("备注").width("40px").build()
    ...
    public void init() {
        ...
        this.initPermissionStatus(AvailableInventoryEntity.class.getSimpleName());
        refreshFirstPage(availableInventoryEntity);
        getPageList();
    }
    ...
    public void simpleSearch() {
        availableInventoryEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(availableInventoryService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(availableInventoryEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedAvailableInventory = availableInventoryEntity.builder().build();
            } else {
                this.selectedAvailableInventory = availableInventoryService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, AvailableInventoryListVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, AvailableInventoryListVM.this, "selectedAvailableInventory");
...
//        this.parentOrgs.clear();
        this.selectedAvailableInventory = null;
        this.showForm = false;
    ...
    @NotifyChange("*")
    public void submit() {
        availableInventoryService.save(this.selectedAvailableInventory);
        this.selectedAvailableInventory = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id) {
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = availableInventoryService.deleteById(id);
                if (bl == true) {
    ...
    public void importData() {
        ...
        this.importEntities = availableInventoryService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, AvailableInventoryEntity.class);
...
////
            ...
            Class<AvailableInventoryEntity> clazz = AvailableInventoryEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(AvailableInventoryEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.availableInventoryEntity.setVehicleModel(null);
        this.availableInventoryEntity.setVin(null);
        this.availableInventoryEntity.setVsn(null);
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/可用库存.xlsx";
            ...
            Filedownload.save(buffer, null, "可用库存.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<AvailableInventoryEntity> dataSet = null;
        ...
        for (AvailableInventoryEntity detailItem : dataSet) {
        ...
        keyList.add("vsn");
        keyList.add("vin");
        keyList.add("inventoryStatus");
        keyList.add("warehouseArea");
        keyList.add("vehicleModel");
        keyList.add("vehicleName");
        keyList.add("color");
        keyList.add("orderId");
        keyList.add("location");
        keyList.add("vehicleNumber");
