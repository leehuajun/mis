...
import com.sunjet.mis.module.warehouse.service.RailwaySendKunMingService;
 ...
 * 中铁发昆明
 */
public class RailwaySendKunMingVM extends ListVM<RailwaySendKunMingEntity> {

    @WireVariable
    private RailwaySendKunMingService railwaySendKunMingService;

    @Getter
    @Setter
    private RailwaySendKunMingEntity railwaySendKunMingEntity = RailwaySendKunMingEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private RailwaySendKunMingEntity selectedRailwaySend;
    ...
    @Getter
    private List<RailwaySendKunMingEntity> importEntities = new ArrayList<>();


    @Init
    public void init() {
        this.setMode(OpenMode.DIALOG);
        this.initPermissionStatus(RailwaySendKunMingEntity.class.getSimpleName());
        refreshFirstPage(railwaySendKunMingEntity);
        getPageList();
    }
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("简写").build(),
            TableHeaderInfo.builder().label("序号").build(),
            TableHeaderInfo.builder().label("火车号").build(),
            TableHeaderInfo.builder().label("VIN").build(),
            TableHeaderInfo.builder().label("合格证编号").build(),
    ...
    public void simpleSearch(){
        railwaySendKunMingEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(railwaySendKunMingService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(railwaySendKunMingEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedRailwaySend = railwaySendKunMingEntity.builder().build();
            } else {
                this.selectedRailwaySend = railwaySendKunMingService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, RailwaySendKunMingVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, RailwaySendKunMingVM.this, "selectedRailwaySend");
...
//        this.parentOrgs.clear();
    ...
    @NotifyChange("*")
    public void submit() {
        railwaySendKunMingService.save(this.selectedRailwaySend);
        this.selectedRailwaySend = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = railwaySendKunMingService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        ...
        this.importEntities = railwaySendKunMingService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, RailwaySendKunMingEntity.class);
...
////
            ...
            Class<RailwaySendKunMingEntity> clazz = RailwaySendKunMingEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(RailwaySendKunMingEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.railwaySendKunMingEntity.setVin("");
        this.railwaySendKunMingEntity.setVariety("");
        this.railwaySendKunMingEntity.setBatchNumber("");
        this.railwaySendKunMingEntity.setVehicleModel("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/中铁发昆明.xlsx";
            ...
            Filedownload.save(buffer, null, "中铁发昆明.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<RailwaySendKunMingEntity> dataSet = null;
