...
import com.sunjet.mis.module.warehouse.service.ProductionWarehouseService;
...
public class ProductionWarehouseListVM extends ListVM<ProductionWarehouseEntity> {
    ...
    @Getter
    @Setter
    private ProductionWarehouseEntity productionWarehouseEntity = ProductionWarehouseEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ProductionWarehouseEntity selectedProductionWarehouse;
    ...
    @Getter
    private List<ProductionWarehouseEntity> importEntities = new ArrayList<>();
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("仓库区域").build(),
            TableHeaderInfo.builder().label("产品型号").build(),
            TableHeaderInfo.builder().label("产品名称").build(),
            TableHeaderInfo.builder().label("颜色").build(),
            TableHeaderInfo.builder().label("订单号").build(),
            TableHeaderInfo.builder().label("库位").build(),
            TableHeaderInfo.builder().label("车辆编号").build(),
            TableHeaderInfo.builder().label("入库日期").build(),
    ...
    public void init() {
        ...
        this.initPermissionStatus(ProductionWarehouseEntity.class.getSimpleName());
        refreshFirstPage(productionWarehouseEntity);
        getPageList();
    }
    ...
    public void simpleSearch(){
        productionWarehouseEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(productionWarehouseService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(productionWarehouseEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedProductionWarehouse = productionWarehouseEntity.builder().build();
            } else {
                this.selectedProductionWarehouse = productionWarehouseService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ProductionWarehouseListVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ProductionWarehouseListVM.this, "selectedProductionWarehouse");
...
//        this.parentOrgs.clear();
        this.selectedProductionWarehouse = null;
        this.showForm = false;
    ...
    @NotifyChange("*")
    public void submit() {
        productionWarehouseService.save(this.productionWarehouseEntity);
        this.selectedProductionWarehouse = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = productionWarehouseService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        ...
        this.importEntities = productionWarehouseService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ProductionWarehouseEntity.class);
...
////
            ...
            Class<ProductionWarehouseEntity> clazz = ProductionWarehouseEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ProductionWarehouseEntity entity) {
    ...
    public void reset() {
       // this.pr.clear();
        this.productionWarehouseEntity.setVsn(null);
        this.productionWarehouseEntity.setVehicleModel(null);
        this.productionWarehouseEntity.setVin(null);
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/生产入库数.xlsx";
            ...
            Filedownload.save(buffer, null, "生产入库数.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ProductionWarehouseEntity> dataSet = null;
        ...
        for (ProductionWarehouseEntity detailItem : dataSet) {
