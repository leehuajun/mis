...
import com.sunjet.mis.module.warehouse.service.ChassisProcurementService;
...
public class ChassisProurementVM extends ListVM<ChassisProcurementEntity> {

    @WireVariable
    private ChassisProcurementService chassisProcurementService;

    @Getter
    @Setter
    private ChassisProcurementEntity chassisProcurementEntity = ChassisProcurementEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ChassisProcurementEntity selectedChassisProcurement;
    ...
    @Getter
    private List<ChassisProcurementEntity> importEntities = new ArrayList<>();


    @Init
    public void init() {
        this.setMode(OpenMode.DIALOG);
        this.initPermissionStatus(ChassisProcurementEntity.class.getSimpleName());
        refreshFirstPage(chassisProcurementEntity);
        getPageList();
    }
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("批次").build(),
            ...
            TableHeaderInfo.builder().label("销售需求").build(),
            TableHeaderInfo.builder().label("配置").build(),
            TableHeaderInfo.builder().label("特殊要求").build()
    ...
    public void simpleSearch(){
        chassisProcurementEntity.setBatchNumber(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(chassisProcurementService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(chassisProcurementEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedChassisProcurement = chassisProcurementEntity.builder().build();
            } else {
                this.selectedChassisProcurement = chassisProcurementService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ChassisProurementVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ChassisProurementVM.this, "selectedChassisProcurement");
...
//        this.parentOrgs.clear();
        this.selectedChassisProcurement = null;
        this.showForm = false;
    ...
    @NotifyChange("*")
    public void submit() {
        chassisProcurementService.save(this.selectedChassisProcurement);
        this.selectedChassisProcurement = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = chassisProcurementService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        ...
        this.importEntities = chassisProcurementService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ChassisProcurementEntity.class);
...
////
            ...
            Class<ChassisProcurementEntity> clazz = ChassisProcurementEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ChassisProcurementEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.chassisProcurementEntity.setVehicleModel("");
        this.chassisProcurementEntity.setVariety("");
        this.chassisProcurementEntity.setBatchNumber("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/底盘采购.xlsx";
            ...
            Filedownload.save(buffer, null, "底盘采购.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ChassisProcurementEntity> dataSet = null;
        ...
        for (ChassisProcurementEntity detailItem : dataSet) {
        ...
        keyList.add("amount");
        keyList.add("variety");
        keyList.add("salesDemand");
        keyList.add("allocation");
        keyList.add("specialRequirements");
