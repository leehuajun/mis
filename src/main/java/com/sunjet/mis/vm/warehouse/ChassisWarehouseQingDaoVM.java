...
import com.sunjet.mis.module.warehouse.service.ChassisWarehouseQingDaoService;
 ...
 * 青岛底盘入库
 */
public class ChassisWarehouseQingDaoVM extends ListVM<ChassisWarehouseQingDaoEntity> {

    @WireVariable
    private ChassisWarehouseQingDaoService chassisWarehouseQingDaoService;

    @Getter
    @Setter
    private ChassisWarehouseQingDaoEntity chassisWarehouseQingDaoEntity = ChassisWarehouseQingDaoEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ChassisWarehouseQingDaoEntity selectedChassis;
    ...
    @Getter
    private List<ChassisWarehouseQingDaoEntity> importEntities = new ArrayList<>();
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("股份生产批号").build(),
            TableHeaderInfo.builder().label("收货批次号").build(),
    ...
    public void init() {
        ...
        this.initPermissionStatus(ChassisWarehouseQingDaoEntity.class.getSimpleName());
        refreshFirstPage(chassisWarehouseQingDaoEntity);
        getPageList();
    }
    ...
    public void simpleSearch(){
        chassisWarehouseQingDaoEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(chassisWarehouseQingDaoService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(chassisWarehouseQingDaoEntity);
        getPageList();
    }
    /**
     * 新增
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedChassis = chassisWarehouseQingDaoEntity.builder().build();
            } else {
                this.selectedChassis = chassisWarehouseQingDaoService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ChassisWarehouseQingDaoVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ChassisWarehouseQingDaoVM.this, "selectedChassis");
...
//        this.parentOrgs.clear();
    ...
    @NotifyChange("*")
    public void submit() {
        chassisWarehouseQingDaoService.save(this.selectedChassis);
        this.selectedChassis = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = chassisWarehouseQingDaoService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        ...
        this.importEntities = chassisWarehouseQingDaoService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ChassisWarehouseQingDaoEntity.class);
...
////
            ...
            Class<ChassisWarehouseQingDaoEntity> clazz = ChassisWarehouseQingDaoEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ChassisWarehouseQingDaoEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.chassisWarehouseQingDaoEntity.setVin("");
        this.chassisWarehouseQingDaoEntity.setVsn("");
        this.chassisWarehouseQingDaoEntity.setSharesProduceNumber("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/青岛底盘入库.xlsx";
            ...
            Filedownload.save(buffer, null, "青岛底盘入库.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ChassisWarehouseQingDaoEntity> dataSet = null;
        ...
        for (ChassisWarehouseQingDaoEntity detailItem : dataSet) {
