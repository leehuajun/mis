...
import com.sunjet.mis.module.warehouse.service.ChassisOutboundLiuZhouService;
 ...
 * 柳州底盘出库
 */
public class ChassisOutboundLiuZhouVM extends ListVM<ChassisOutboundLiuZhouEntity> {

    @WireVariable
    private ChassisOutboundLiuZhouService chassisOutboundLiuZhouService;

    @Getter
    @Setter
    private ChassisOutboundLiuZhouEntity chassisOutboundLiuZhouEntity = ChassisOutboundLiuZhouEntity.builder().build();
    @Getter
    @Setter
    private Boolean showForm = false;
    @Getter
    @Setter
    private ChassisOutboundLiuZhouEntity selectedChassis;
    ...
    @Getter
    private List<ChassisOutboundLiuZhouEntity> importEntities = new ArrayList<>();
    ...
    private List<TableHeaderInfo> tableHeaderList = Arrays.asList(
            ...
            TableHeaderInfo.builder().label("班次").build(),
            TableHeaderInfo.builder().label("产品名称").build(),
            TableHeaderInfo.builder().label("扫描点代码").build(),
            TableHeaderInfo.builder().label("扫描点名称").build(),
            TableHeaderInfo.builder().label("过站时间").build(),
            TableHeaderInfo.builder().label("扫描人员").build(),
    ...
    public void init() {
        ...
        this.initPermissionStatus(ChassisOutboundLiuZhouEntity.class.getSimpleName());
        refreshFirstPage(chassisOutboundLiuZhouEntity);
        getPageList();
    }
    ...
    public void simpleSearch(){
        chassisOutboundLiuZhouEntity.setVin(this.getKeyword());
        getPageList();
    }
...
//    @Command
    public void getPageList() {
        this.setPageResult(chassisOutboundLiuZhouService.getPageList(this.getPageParam()));
    }

    @Command
    @NotifyChange("pageResult")
    public void gotoPageNo(@BindingParam("e") Event event) {
        refreshPage(chassisOutboundLiuZhouEntity);
        getPageList();
    }
    ...
    protected void openDialog(String id) {
        try {
            ...
            if (StringUtils.isBlank(id)) {
                this.selectedChassis = chassisOutboundLiuZhouEntity.builder().build();
            } else {
                this.selectedChassis = chassisOutboundLiuZhouService.findById(id);
            }
            this.showForm = true;
            BindUtils.postNotifyChange(null, null, ChassisOutboundLiuZhouVM.this, "showForm");
            BindUtils.postNotifyChange(null, null, ChassisOutboundLiuZhouVM.this, "selectedChassis");
...
//        this.parentOrgs.clear();
    ...
    @NotifyChange("*")
    public void submit() {
        chassisOutboundLiuZhouService.save(this.selectedChassis);
        this.selectedChassis = null;
        this.showForm = false;
    ...
    public void confirmDeleteObject(@BindingParam("id") String id){
        ZkUtils.showQuestion("如果此单据已经打开将会被关闭\n是否删除此单据？", "询问", event -> {
            ...
            if (clickedButton == Messagebox.OK) {
                boolean bl = chassisOutboundLiuZhouService.deleteById(id);
                if(bl==true){
    ...
    public void importData() {
        ...
        this.importEntities = chassisOutboundLiuZhouService.saveAll(this.importEntities);
...
//            ZkUtils.showInformation(path + fileName, "转义后的文件名");
            ...
            importEntities = ExcelUtil.readExcel(path + fileName, ChassisOutboundLiuZhouEntity.class);
...
////
            ...
            Class<ChassisOutboundLiuZhouEntity> clazz = ChassisOutboundLiuZhouEntity.class;
...
//            System.out.println("新文件的编码格式: " + EncodingDetect.getJavaEncodeFile(path + fileName));
    ...
    public String genStyle(ChassisOutboundLiuZhouEntity entity) {
    ...
    @NotifyChange("*")
    public void reset() {
        this.chassisOutboundLiuZhouEntity.setVin("");
        this.getPageList();
    }
    ...
    public void downloadTemplate() {
        try {
            byte[] buffer = null;
            String path = ClassUtils.getDefaultClassLoader().getResource("").getPath() + "template/柳州底盘出库.xlsx";
            ...
            Filedownload.save(buffer, null, "柳州底盘出库.xlsx");
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void export(@BindingParam("type") String type) {
        ...
        //当前页
        List<ChassisOutboundLiuZhouEntity> dataSet = null;
        ...
        for (ChassisOutboundLiuZhouEntity detailItem : dataSet) {
        ...
        keyList.add("scanCode");
        keyList.add("scanName");
